...
- [claudectl](https://github.com/mercurialsolo/claudectl) by [mercurialsolo](https://github.com/mercurialsolo) - Auto-pilot for Claude Code with a local LLM brain (ollama/llama.cpp/vLLM) that learns from your corrections to auto-approve/deny tool calls. Multi-session orchestration with dependency ordering, cross-session context routing, file conflict detection, cognitive rot monitoring, spend control, and session highlight reel recording. Sub-1MB Rust binary, sub-50ms startup, 7 runtime deps, fully local with zero telemetry.
